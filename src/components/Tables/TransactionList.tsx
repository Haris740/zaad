...
const TransactionList = ({
  ...
  const { data: paymentData, isLoading } = useQuery({
    queryKey: ["payment", pageNumber, type, id, filter, showEmployee],
    queryFn: async () => {
      const res = await axios.get(
        `/api/payment${type
          ? "/" + (type === "self" ? type : type + "/" + id)
          : ""
        }?page=${pageNumber}${query}&showEmployee=${showEmployee}`
      );
      return res.data;
    },
  ...
  const handleCreateInvoice = async (invoiceMeta: any) => {
    const selectedRecords = recordsWithBalance.filter((r) =>
    ...
    const invoicePayload = {
      ...
      items,
      advance,
      createdFromTransactions: Array.from(selectedIds),
    };

    try {
      const { data } = await axios.post("/api/invoice", invoicePayload);
      setInvoiceModalOpen(false);
      clearSelection();

      // redirect to invoice
      window.location.href = `/accounts/invoice/${data._id}`;
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const RecordRow = ({
    ...
    return (
      <div
        className={clsx(
          "grid grid-cols-3 sm:grid-cols-8",
          type && "sm:grid-cols-9",
          index % 2 !== 0 && "bg-gray dark:bg-slate-800",
          index !== totalLength - 1 &&
      ...
      >
        ...
        <div className="flex items-center gap-2 p-2.5 xl:p-5">
          ...
          />
          <div>
            <p className="hidden sm:block uppercase text-black dark:text-white">
              {(record.suffix || "") + (record.number || "")}
            </p>
            {record.edited && (
              <p className="hidden sm:block text-xs uppercase text-red">Edited</p>
            )}
          </div>
        </div>
        ...
        {/* Client */}
        <Link
          href={`/${record.client.type !== "self"
            ? `${record.client.type}/${record.client.id}`
            : "accounts/transactions/self"
            }`}
          className="flex items-center gap-3 p-2.5 xl:p-5"
        >
          <p className="hidden sm:block capitalize text-black dark:text-white">
            {record.client.name}
          </p>
        </Link>

        {/* Particular */}
        <div className="hidden sm:flex items-center p-2.5 xl:p-5">
        ...
        <div className="flex items-center justify-center p-2.5 xl:p-5">
          {record.method}
          {record.status && (
            <span className="ml-2 rounded-md border bg-opacity-10 px-1 text-sm text-meta-5">
              {record.status}
            </span>
          )}
        </div>
          ...
          {record.type === "expense" &&
            ...
            record.serviceFee != 0 && (
              <div className="text-center">
                <p
                  className={clsx(
                    +record.serviceFee > 0 ? "bg-meta-3" : "bg-red",
                    "rounded-md px-2 text-white dark:text-black"
                  )}
                >
                  {record.serviceFee} <span className="text-xs">AED</span>
                </p>
        ...
        {type && (
          <div className="flex items-center justify-center p-2.5 xl:p-5">
            <p
              className={clsx(
                record.runningBalance >= 0 ? "text-meta-3" : "text-red"
              )}
            >
        ...
        <div className="flex items-center justify-center gap-1">
          {!type && !id && (
            <Link
              href={`/accounts/transactions/${record.client.type !== "self"
                ? `${record.client.type}/${record.client.id}`
                : "self"
                }`}
              className="rounded p-1 hover:bg-slate-500 hover:bg-opacity-10"
            >
          ...
          <button
            ...
            className="rounded p-1 hover:bg-red hover:bg-opacity-10"
          >
            <img src="/images/icon/bin.svg" />
          </button>
        </div>
      </div>
    );
  ...
  return (
    <>
              ...
              type === "company" && (
                <div className="flex items-center gap-2 ml-3">
                  <button
                    ...
                    className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${showEmployee ? "bg-primary" : "bg-[#dbdbdb]"
                      }`}
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${showEmployee ? "translate-x-4" : "translate-x-1"
                        }`}
                    />
                  </button>
            ...
            <div className="p-2.5 xl:p-5 flex items-center gap-2">
              <input
                ...
                checked={
                  ...
                  recordsWithBalance.every((r) => selectedIds.has(r.id))
                }
                onChange={(e) => {
                  if (e.target.checked) {
                    setSelectedIds(new Set(recordsWithBalance.map((r) => r.id)));
                  } else {
                    clearSelection();
                  }
                }}
            ...
            {isLoading ? (
              ...
              recordsWithBalance?.map((record, index) => (
                <RecordRow
                  ...
                  type={type}
                  id={id}
                  totalLength={recordsWithBalance.length}
                />
              ))
            )}
          </div>
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-999 bg-white dark:bg-boxdark border-t border-stroke px-6 py-3 shadow-lg">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <button
                // onClick={() => {
                //   setSelectedRecordId([...selectedIds][0]);
                //   setIsConfirmationOpen(true);
                // }}
                className="px-4 py-2 rounded bg-red text-white hover:bg-red-600"
              >
                Delete Selected
              </button>
